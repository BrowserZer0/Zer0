...
class Settings extends React.Component {
  ...
  componentDidMount () {
    ...
    // Listen for Tor state changes for instant updates
    window.preloadAPI.receive('torStateChanged', (isTorEnabled) => {
      console.log('[SETTINGS] Received torStateChanged event, isTorEnabled:', isTorEnabled)
      var pref = { ...this.state.pref }
      pref.isTorEnabled = isTorEnabled
      this.setState({ pref, torRestarting: false })
    })
  }
  ...
  openUrl = url => {
    const newtab = this.props.tabGroup.addTab({
      src: url,
      title: 'Loading...',
      isNative: false
    })
        ...
        <div className='row pt-4'>
          <div className='sidebar col-sm-3 col-xs-3 vh-100 pt-2 ml-5 pl-4 mr-3 border-right border-white'>
            ...
            <div
              ...
              role='button'
              onClick={() => {
                this.openUrl('https://zer0.build/')
              }}
            >
              <img className='logo' src={process.env.PUBLIC_URL + '/icon_zero.png'} alt='iconzero' />
            ...
            >
              ...
              <div className=''>
                ...
                <button
                  ...
                  disabled={this.state.torRestarting}
                  onClick={() => {
                    var pref = { ...this.state.pref }
                    if (!pref.isTorEnabled) {
                      pref.isTorEnabled = true
                      this.setState({ pref, torRestarting: true })
                      // Send toggle request immediately without restart
                      window.preloadAPI.send('torWindow')
                    }
                  }}
                >
                  {this.state.torRestarting && !this.state.pref.isTorEnabled
                    ? 'Activating...'
                    : 'Activate'}
                </button>
                <button
